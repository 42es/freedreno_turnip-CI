...
apply_patches() {
	...
	for patch in "${arr[@]}"; do
		...
		if [[ $patch_source == *"../.."* ]]; then
			...
			curl --output "../$patch_file".patch -k --retry-delay 30 --retry 5 -f --retry-all-errors https://gitlab.freedesktop.org/mesa/mesa/src/-/"$patch_source".patch
			sleep 1
...
patch_to_description() {
	...
	for patch in "${arr[@]}"; do
		...
		if [[ $patch_source == *"../.."* ]]; then
			...
			echo "- $patch_name, [$patch_source](https://gitlab.freedesktop.org/mesa/mesa/src/-/$patch_source), $patch_args" >> description
		fi
	done
}
